...
const API_BASE_URL = 'https://mash-backend-api-production.up.railway.app/api/v1';
...
async function testResendCode() {
  ...
  try {
    const response = await makeRequest('/auth/resend-verification-code', 'POST', {
      email: TEST_EMAIL
    });
...
async function runDiagnostics() {
  ...
  if (TEST_EMAIL === 'your-test-email@example.com') {
    console.log('⚠️  WARNING: Please update TEST_EMAIL with your actual email address!\n');
  }
  ...
  if (registrationWorks) {
    ...
    console.log('   - AWS_SES_ACCESS_KEY / AWS_SES_SECRET_KEY (if using AWS SES)');
    console.log('   - SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS (if using SMTP)');
